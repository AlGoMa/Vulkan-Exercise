...
namespace App::Math {
    ...
    class Quaternion : 
        public Common::Resources::IResource<Quaternion>
    {
    public:
        ...
        friend Quaternion   operator +(const Quaternion& in_qFirstQuaterion, const Quaternion& in_qSecondQuaternion);
        inline float&       operator [](const complex in_eAxis) noexcept { return m_vAxis[static_cast<t_IntU8>(in_eAxis)]; }
        inline const float& operator [](const complex in_eAxis) const noexcept { return const_cast<Vector&>(m_vAxis)[static_cast<t_IntU8>(in_eAxis)]; }
        ...
        Quaternion operator *=(const Quaternion& in_qB) { 
            ...
            return Quaternion (vCross + Vector(in_qB[complex::i] * m_vAxis[complex::t] + in_qB[complex::t] * m_vAxis[complex::i],
                                               ...
                                               m_vAxis[complex::t] * in_qB[complex::t] - (Vector::Dot(Vector(m_vAxis, 0.0f), Vector(in_qB.m_vAxis, 0.0f)))));
        }
