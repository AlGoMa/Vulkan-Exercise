...
namespace App::FiniteStateMachine
{
    class State;
    class Event;

    class FSM final :
        Common::Resources::IResource<FSM>
    {
    public:
        FSM(void);
    ...
    class Event :
        public Common::Resources::IResource<Event>
    {
    public:
        Event(_In_ _Notnull_ std::string in_strIdentifier) :
            Common::Resources::IResource<Event>(in_strIdentifier) {}
        Event(void) :
            Common::Resources::IResource<Event>(Common::Resources::ClassName(typeid(Event).name())) {}
        ~Event(void) {}
    };

    class State :
        public Common::Resources::IResource<State>
    {
    public:
        State(_In_ _Notnull_ std::string in_strIdentifier, _In_opt_ _Maybenull_ FSM* in_pOwner ) : Common::Resources::IResource<State>(in_strIdentifier),
            m_pOwner(in_pOwner),
            m_unSubState(0)
        {
        }
        State(void) : Common::Resources::IResource<State>(Common::Resources::ClassName(typeid(State).name())),
            m_unSubState(0),
            m_pOwner(nullptr)
        {
        }
    public:
        ...
        void Dispose(void) override
        {
            if (0 < m_unSubState && nullptr != m_pOwner)
            {
        ...
        const t_IntU64 GetSuperState(void)
        {
            ...
            return nullptr != out_pSSID ? out_pSSID->GetHashID() : 0;
        }
